...
@router.get("/uploads-disk-diag", dependencies=[Depends(require_service_key)])
def uploads_disk_diag():
    """Diagnostico temporario: quanto espaco as pastas 'originais' (foto
    bruta, so usada pra recuperar peca travada em 'processing') estao
    ocupando no volume /data/uploads, que bateu no limite (500MB) e travou
    upload novo com OSError: No space left on device."""
    from pathlib import Path
    base = Path(settings.uploads_dir)
    if not base.exists():
        return {"error": f"{base} nao existe"}
    ...
    total_geral = du(base)
    total_originais = 0
    n_pastas_originais = 0
    for part_dir in base.iterdir():
        if not part_dir.is_dir():
            continue
        originais = part_dir / "originais"
        if originais.exists():
            total_originais += du(originais)
            n_pastas_originais += 1

    return {
        "total_geral_mb": round(total_geral / 1024 / 1024, 1),
        "total_originais_mb": round(total_originais / 1024 / 1024, 1),
        "pastas_com_originais": n_pastas_originais,
    }


@router.post("/uploads-cleanup-originais", dependencies=[Depends(require_service_key)])
def uploads_cleanup_originais(db: Session = Depends(get_db)):
    """Apaga a pasta 'originais' (foto bruta pre-otimizacao) das pecas que
    JA tem fotos otimizadas prontas (part.photos preenchido) e NAO estao em
    status=processing — pra essas, a pasta originais so serve de backup pra
    recuperar upload travado no meio, o que ja passou. Libera espaco no
    volume sem afetar nenhuma peca em andamento."""
    from pathlib import Path
    import shutil
    base = Path(settings.uploads_dir)
    parts = db.query(Part).filter(Part.status != "processing").all()
    freed = 0
    removed = 0
    for part in parts:
        if not part.photos:
            continue
        originais = base / str(part.id) / "originais"
        if originais.exists():
            for f in originais.rglob("*"):
                if f.is_file():
                    freed += f.stat().st_size
            shutil.rmtree(originais, ignore_errors=True)
            removed += 1
